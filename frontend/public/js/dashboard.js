...
document.addEventListener('DOMContentLoaded', function() {
  ...
  initializeCommunityOverviewCharts();
...
function seedStatisticsDemoAlerts() {
  if (!Array.isArray(alertsData)) alertsData = [];

  const hasSyntheticSeed = alertsData.some(a => String(a?.id || '').startsWith('STAT-'));
  if (hasSyntheticSeed) return;

  const now = Date.now();
  const neighborhoods = [
    { name: 'Leblon', location: 'Av. Bartolomeu Mitre', weight: 1.25 },
    { name: 'Ipanema', location: 'R. Barão da Torre', weight: 1.05 },
    { name: 'Copacabana', location: 'Av. Nossa Senhora de Copacabana', weight: 1.15 },
    { name: 'Jardim Botânico', location: 'R. Jardim Botânico', weight: 0.9 }
  ];
  const types = [
    { key: 'danger', titles: ['Ocorrência de segurança', 'Movimentação suspeita', 'Ação policial em andamento'], statuses: ['novo', 'investigando', 'novo'] },
    { key: 'warning', titles: ['Bloqueio viário parcial', 'Acidente com retenção', 'Intervenção urbana temporária'], statuses: ['investigando', 'novo', 'investigando'] },
    { key: 'info', titles: ['Atitude suspeita reportada', 'Ponto de atenção monitorado', 'Fluxo incomum na região'], statuses: ['novo', 'investigando', 'novo'] },
    { key: 'success', titles: ['Ocorrência finalizada', 'Rota liberada', 'Situação normalizada'], statuses: ['resolvido', 'resolvido', 'investigando'] }
  ];

  const syntheticAlerts = [];
  for (let day = 0; day < 365; day += 1) {
    const recentBoost = day < 7 ? 7 : day < 30 ? 4 : day < 90 ? 2 : 1;
    const wave = ((day % 14) < 4) ? 2 : ((day % 9) === 0 ? 1 : 0);
    const weekendPenalty = (day % 7 === 0 || day % 7 === 6) ? -1 : 0;
    neighborhoods.forEach((neighborhood, neighborhoodIndex) => {
      const neighborhoodPulse = ((day + neighborhoodIndex) % 5 === 0) ? 2 : 0;
      const volume = Math.max(1, Math.round((recentBoost + wave + weekendPenalty + neighborhoodPulse) * neighborhood.weight));
      for (let itemIndex = 0; itemIndex < volume; itemIndex += 1) {
        const typeMeta = types[(day + itemIndex + neighborhoodIndex) % types.length];
        const title = typeMeta.titles[(day + itemIndex) % typeMeta.titles.length];
        const status = typeMeta.statuses[(day + itemIndex + neighborhoodIndex) % typeMeta.statuses.length];
        const hour = (6 + (day * 3 + itemIndex * 5 + neighborhoodIndex * 4)) % 24;
        const minute = (day * 11 + itemIndex * 13 + neighborhoodIndex * 7) % 60;
        const timestamp = new Date(now - (day * 24 * 60 * 60 * 1000) - (hour * 60 * 60 * 1000) - (minute * 60 * 1000)).toISOString();

        syntheticAlerts.push({
          id: `STAT-${day}-${neighborhoodIndex}-${itemIndex}`,
          type: typeMeta.key,
          title,
          neighborhood: neighborhood.name,
          location: `${neighborhood.name} • ${neighborhood.location}`,
          status,
          timestamp,
          description: `${title} registrado em ${neighborhood.name}.`
        });
      }
    });
  }
  ...
  alertsData = [
    ...syntheticAlerts,
    ...alertsData.filter(a => !String(a?.id || '').startsWith('STAT-'))
  ];
}
...
function initializeCommunityOverviewCharts() {
  const neighborhoodCanvas = document.getElementById('communityNeighborhoodGrowthChart');
  if (neighborhoodCanvas) {
    new Chart(neighborhoodCanvas.getContext('2d'), {
      type: 'bar',
      data: {
        labels: ['Leblon', 'Jd. Botânico', 'Copacabana', 'Ipanema', 'Lagoa'],
        datasets: [{
          label: 'Novos cadastros',
          data: [18, 14, 11, 9, 7],
          backgroundColor: [
            'rgba(78, 115, 223, 0.86)',
            'rgba(54, 185, 204, 0.86)',
            'rgba(28, 200, 138, 0.86)',
            'rgba(246, 194, 62, 0.9)',
            'rgba(133, 135, 150, 0.82)'
          ],
          borderRadius: 10,
          borderSkipped: false,
          maxBarThickness: 34
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: function(context) { return `${context.raw} novos cadastros`; }
            }
          }
        },
        scales: {
  ...
  const channelsCanvas = document.getElementById('communityChannelsChart');
  if (channelsCanvas) {
    new Chart(channelsCanvas.getContext('2d'), {
      type: 'doughnut',
      data: {
        labels: ['App', 'E-mail', 'WhatsApp', 'Portal web'],
        datasets: [{
          data: [48, 22, 18, 12],
          backgroundColor: [
            'rgba(78, 115, 223, 0.92)',
            'rgba(28, 200, 138, 0.9)',
            'rgba(246, 194, 62, 0.92)',
            'rgba(133, 135, 150, 0.88)'
          ],
          borderColor: '#ffffff',
          borderWidth: 4,
          hoverOffset: 8
...
function initializeStatisticsCharts() { const emptyData = { labels: [], datasets: [] }; let ctxHour = document.getElementById('alertsByHourChart')?.getContext('2d'); if (ctxHour) { alertsByHourChart = new Chart(ctxHour, { type: 'line', data: emptyData, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } } }); } let ctxDay = document.getElementById('alertsByDayChart')?.getContext('2d'); if (ctxDay) { alertsByDayChart = new Chart(ctxDay, { type: 'bar', data: emptyData, options: { responsive: true, maintainAspectRatio: false, indexAxis: 'y', plugins: { legend: { display: false } } } }); } let ctxStatus = document.getElementById('alertsStatusChart')?.getContext('2d'); if(ctxStatus) { alertsStatusChart = new Chart(ctxStatus, { type: 'doughnut', data: emptyData, plugins: [doughnutTextPlugin], options: { responsive: true, maintainAspectRatio: false, cutout: '68%', plugins: { legend: { position: 'bottom' } } } }); } let ctxTypeNeighborhood = document.getElementById('alertsByTypeAndNeighborhoodChart')?.getContext('2d'); if(ctxTypeNeighborhood) { alertsByTypeAndNeighborhoodChart = new Chart(ctxTypeNeighborhood, { type: 'bar', data: emptyData, options: { responsive: true, maintainAspectRatio: false, scales: { x: { stacked: true }, y: { stacked: true, beginAtZero: true, max: 100 } }, plugins: { legend: { position: 'bottom' } } } }); } }
function getStatisticsSourceData() {
  seedStatisticsDemoAlerts();
  return Array.isArray(alertsData) ? alertsData.filter(Boolean) : [];
}

function populateStatisticsNeighborhoodFilter() {
  const neighborhoodFilter = document.getElementById('stats-neighborhood-filter');
  if (!neighborhoodFilter) return;
  const currentValue = neighborhoodFilter.value || 'all';
  neighborhoodFilter.innerHTML = '<option value="all">Todos os bairros</option>';
  const neighborhoods = [...new Set(getStatisticsSourceData().map(getAlertNeighborhood).filter(Boolean))]
  ...
  neighborhoodFilter.value = neighborhoods.includes(currentValue) ? currentValue : 'all';
}

function updateStatisticsFiltersSummary(data, filters) {
  const summary = document.getElementById('stats-filters-summary');
  if (!summary) return;

  const parts = [];
  parts.push(filters.periodDays === 365 ? 'último ano' : `últimos ${filters.periodDays} dias`);
  if (filters.neighborhood !== 'all') parts.push(filters.neighborhood);

  const typeLabels = {
    danger: 'segurança / crime',
    warning: 'trânsito / urbano',
    info: 'suspeito / atenção',
    success: 'finalizados'
  };
  const statusLabels = {
    novo: 'novos',
  ...
  if (filters.type !== 'all') parts.push(typeLabels[filters.type] || filters.type);
  if (filters.status !== 'all') parts.push(statusLabels[filters.status] || filters.status);

  const scope = parts.length ? parts.join(' • ') : 'todos os filtros';
  const total = data.length;
  summary.textContent = total === 1
    ? `Exibindo 1 alerta para ${scope}.`
    : `Exibindo ${total} alertas para ${scope}.`;
}


function forceStatisticsChartsResize() {
  const charts = [alertsByHourChart, alertsByDayChart, alertsByTypeAndNeighborhoodChart, alertsStatusChart].filter(Boolean);
...
function refreshStatisticsPanel() {
  ...
  if (!periodFilter || !neighborhoodFilter || !typeFilter || !statusFilter) return;

  populateStatisticsNeighborhoodFilter();

  const periodDays = parseInt(periodFilter.value, 10) || 7;
  ...
  let filteredData = getStatisticsSourceData().filter(a => {
    const d = getAlertDate(a);
    return d && d >= startDate;
  });

  if (neighborhood !== 'all') {
    filteredData = filteredData.filter(a => getAlertNeighborhood(a) === neighborhood);
  }

  if (type !== 'all') {
    filteredData = filteredData.filter(a => String(a?.type || '').toLowerCase() === type);
  }

  if (status !== 'all') {
    filteredData = filteredData.filter(a => String(a?.status || '').toLowerCase() === status);
  }

  updateStatsCards(filteredData, periodDays);
  try { updateAlertsByHourChart(filteredData); } catch (error) { console.warn('Falha ao atualizar alertsByHourChart', error); }
  try { updateAlertsByDayChart(filteredData, periodDays); } catch (error) { console.warn('Falha ao atualizar alertsByDayChart', error); }
  try { updateAlertsByTypeAndNeighborhoodChart(filteredData, periodDays); } catch (error) { console.warn('Falha ao atualizar alertsByTypeAndNeighborhoodChart', error); }
  try { updateAlertsStatusChart(filteredData, periodDays); } catch (error) { console.warn('Falha ao atualizar alertsStatusChart', error); }
  updateStatisticsFiltersSummary(filteredData, { periodDays, neighborhood, type, status });
  forceStatisticsChartsResize();
}



function applyStatisticsFiltersFromUI() {
  const periodFilter = document.getElementById('stats-period-filter');
  const neighborhoodFilter = document.getElementById('stats-neighborhood-filter');
  const typeFilter = document.getElementById('stats-type-filter');
  const statusFilter = document.getElementById('stats-status-filter');
  ...
  if (!periodFilter || !neighborhoodFilter || !typeFilter || !statusFilter) return false;
  ...
  if (summary) {
    summary.textContent = `Atualizando gráficos para ${periodDays === 365 ? 'último ano' : `últimos ${periodDays} dias`}...`;
  }

  refreshStatisticsPanel();

  if (summary) {
    const currentData = getFilteredStatisticsDataFromUI();
    updateStatisticsFiltersSummary(currentData, { periodDays, neighborhood, type, status });
  }
...
function getFilteredStatisticsDataFromUI() {
  const periodFilter = document.getElementById('stats-period-filter');
  const neighborhoodFilter = document.getElementById('stats-neighborhood-filter');
  const typeFilter = document.getElementById('stats-type-filter');
  const statusFilter = document.getElementById('stats-status-filter');
  ...
  if (neighborhood !== 'all') filteredData = filteredData.filter(a => getAlertNeighborhood(a) === neighborhood);
  if (type !== 'all') filteredData = filteredData.filter(a => String(a?.type || '').toLowerCase() === type);
  if (status !== 'all') filteredData = filteredData.filter(a => String(a?.status || '').toLowerCase() === status);
  return filteredData;
}
...
function setupStatisticsFilters() {
  ...
  const typeFilter = document.getElementById('stats-type-filter');
  const statusFilter = document.getElementById('stats-status-filter');
  const applyButton = document.getElementById('stats-filters-apply');
  const filtersForm = document.getElementById('stats-filters-form');
  if (!periodFilter || !neighborhoodFilter || !typeFilter || !statusFilter) return;

  periodFilter.value = periodFilter.value || '7';
  populateStatisticsNeighborhoodFilter();

  const getPendingFiltersSummary = () => {
    const periodDays = parseInt(periodFilter.value, 10) || 7;
    const parts = [periodDays === 365 ? 'último ano' : `últimos ${periodDays} dias`];
    if ((neighborhoodFilter.value || 'all') !== 'all') parts.push(neighborhoodFilter.value);
    if ((typeFilter.value || 'all') !== 'all') parts.push(typeFilter.options[typeFilter.selectedIndex]?.text || typeFilter.value);
    if ((statusFilter.value || 'all') !== 'all') parts.push(statusFilter.options[statusFilter.selectedIndex]?.text || statusFilter.value);
    return `Pesquisa pronta para ${parts.join(' • ')}. Clique em Pesquisar para aplicar.`;
  };

  const updatePendingSummary = () => {
    const summary = document.getElementById('stats-filters-summary');
    if (summary) summary.textContent = getPendingFiltersSummary();
  };

  const runStatisticsSearch = (event) => {
    if (event) {
  ...
  window.runStatisticsSearchNow = applyStatisticsFiltersFromUI;
  window.handleStatisticsSearchClick = applyStatisticsFiltersFromUI;
  window.applyStatisticsFiltersFromUI = applyStatisticsFiltersFromUI;
  window.updatePendingStatisticsSummary = updatePendingSummary;

  if (filtersForm && !filtersForm.dataset.bound) {
    filtersForm.addEventListener('submit', applyStatisticsFiltersFromUI);
    filtersForm.dataset.bound = 'true';
  }

  if (applyButton && !applyButton.dataset.bound) {
    applyButton.setAttribute('type', 'button');
    applyButton.onclick = applyStatisticsFiltersFromUI;
    applyButton.addEventListener('click', applyStatisticsFiltersFromUI);
    applyButton.dataset.bound = 'true';
  }

  [periodFilter, neighborhoodFilter, typeFilter, statusFilter].forEach((el) => {
    if (!el.dataset.bound) {
      el.addEventListener('change', updatePendingSummary);
...
function updateStatsCards(data, periodDays) { document.getElementById('stats-total-alerts').textContent = data.length; document.getElementById('stats-daily-average').textContent = (data.length / Math.max(periodDays, 1)).toFixed(1); const hourCounts = data.reduce((acc, a) => { const date = getAlertDate(a); if (!date) return acc; const hour = date.getHours(); acc[hour] = (acc[hour] || 0) + 1; return acc; }, {}); const peakHour = Object.keys(hourCounts).length ? Object.keys(hourCounts).reduce((a, b) => hourCounts[a] > hourCounts[b] ? a : b) : 'N/A'; document.getElementById('stats-peak-hour').textContent = peakHour !== 'N/A' ? `${peakHour}:00` : 'N/A'; const neighborhoodCounts = data.reduce((acc, a) => { const n = getAlertNeighborhood(a); if (!n) return acc; acc[n] = (acc[n] || 0) + 1; return acc; }, {}); const topNeighborhood = Object.keys(neighborhoodCounts).length ? Object.keys(neighborhoodCounts).reduce((a, b) => neighborhoodCounts[a] > neighborhoodCounts[b] ? a : b) : 'N/A'; document.getElementById('stats-top-neighborhood').textContent = topNeighborhood; }
function updateAlertsByHourChart(data) { if (!alertsByHourChart) return; const buckets = [
  { label: 'Madrugada', start: 0, end: 5 },
  { label: 'Manhã', start: 6, end: 11 },
  { label: 'Tarde', start: 12, end: 17 },
  { label: 'Noite', start: 18, end: 23 }
]; const bucketCounts = buckets.map(() => 0); data.forEach(a => { const date = getAlertDate(a); if (!date) return; const hour = date.getHours(); const index = buckets.findIndex(bucket => hour >= bucket.start && hour <= bucket.end); if (index >= 0) bucketCounts[index]++; }); const ctx = alertsByHourChart.ctx; const gradient = ctx.createLinearGradient(0, 0, 0, alertsByHourChart.height); gradient.addColorStop(0, 'rgba(99, 102, 241, 0.5)'); gradient.addColorStop(1, 'rgba(99, 102, 241, 0)'); alertsByHourChart.data = { labels: buckets.map(bucket => bucket.label), datasets: [{ label: 'Alertas por faixa do dia', data: bucketCounts, backgroundColor: gradient, borderColor: 'rgba(99, 102, 241, 1)', fill: true, tension: 0.35, pointRadius: 3, pointBackgroundColor: 'rgba(99, 102, 241, 1)' }] }; alertsByHourChart.update(); }
function updateAlertsByDayChart(data, periodDays = 7) {
  if (!alertsByDayChart) return;
  ...
  if (periodDays <= 7) {
    for (let i = periodDays - 1; i >= 0; i -= 1) {
      ...
      counts.push(data.filter(a => {
        const date = getAlertDate(a);
    ...
    for (let i = 2; i >= 0; i -= 1) {
      ...
      labels.push(start.toLocaleDateString('pt-BR', { month: 'short' }));
      counts.push(data.filter(a => {
        const date = getAlertDate(a);
        return date && date >= start && date <= end;
      }).length);
  ...
  alertsByDayChart.options.indexAxis = 'x';
  alertsByDayChart.options.scales = alertsByDayChart.options.scales || {};
  alertsByDayChart.options.scales.y = { beginAtZero: true, ticks: { precision: 0 } };
  alertsByDayChart.options.plugins = alertsByDayChart.options.plugins || {};
  alertsByDayChart.options.plugins.tooltip = { callbacks: { label: (context) => `${context.raw} alerta${context.raw === 1 ? '' : 's'}` } };
  alertsByDayChart.update();
}
function updateAlertsStatusChart(data, periodDays = 7) {
  const canvas = document.getElementById('alertsStatusChart');
  ...
  const counts = { 'Novo': 0, 'Em Investigação': 0, 'Resolvido': 0 };
  (Array.isArray(data) ? data : []).forEach(a => {
    const status = statusMap[String(a?.status || '').toLowerCase()] || 'Novo';
    counts[status] = (counts[status] || 0) + 1;
  });

  let labels = Object.keys(counts);
  let totals = labels.map(label => counts[label]);
  ...
  if (alertsStatusChart) {
    try { alertsStatusChart.destroy(); } catch (error) {}
  }

  alertsStatusChart = new Chart(canvas.getContext('2d'), {
    type: 'doughnut',
    data: {
      labels,
      datasets: [{
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label(context) {
              if (totalCount === 0) return 'Sem ocorrências para os filtros atuais';
              const value = context.raw || 0;
              return `${context.label}: ${value} alerta${value === 1 ? '' : 's'}`;
            }
          }
        },
        title: {
    ...
    plugins: [{
      ...
      afterDraw(chart) {
        ...
        const x = first ? first.x : chart.width / 2;
        const y = first ? first.y : chart.height / 2;
        const ctx = chart.ctx;
        ctx.save();
        ...
        ctx.fillText(totalCount === 0 ? '0' : String(totalCount), x, y - 8);
        ctx.fillStyle = '#64748b';
        ctx.font = '12px Arial';
        ctx.fillText(totalCount === 0 ? 'Sem dados' : (periodDays === 365 ? 'Último ano' : `${periodDays} dias`), x, y + 14);
        ctx.restore();
      }
    }]
  });
...
function updateAlertsByTypeAndNeighborhoodChart(data, periodDays = 7) {
  const canvas = document.getElementById('alertsByTypeAndNeighborhoodChart');
  if (!canvas || typeof Chart === 'undefined') return;

  const ctx = canvas.getContext('2d');
  const typeMap = {
    danger: 'Crime',
    warning: 'Acidente/Urbano',
    info: 'Suspeito',
    success: 'Resolvido'
  };
  const baseColors = {
    'Crime': '#EF4444',
    'Acidente/Urbano': '#F59E0B',
    'Suspeito': '#06B6D4',
    'Resolvido': '#10B981'
  };

  const preferredNeighborhoods = ['Leblon', 'Ipanema', 'Copacabana', 'Jardim Botânico'];
  const preferredTypes = ['Crime', 'Acidente/Urbano', 'Suspeito', 'Resolvido'];
  const rawCounts = Object.fromEntries(preferredNeighborhoods.map(n => [n, Object.fromEntries(preferredTypes.map(t => [t, 0]))]));

  (Array.isArray(data) ? data : []).forEach(a => {
    const neighborhood = getAlertNeighborhood(a);
    const type = typeMap[String(a?.type || '').toLowerCase()] || 'Suspeito';
    if (!neighborhood) return;
    if (!rawCounts[neighborhood]) {
      rawCounts[neighborhood] = Object.fromEntries(preferredTypes.map(t => [t, 0]));
    }
    rawCounts[neighborhood][type] = (rawCounts[neighborhood][type] || 0) + 1;
  });

  const labels = Object.keys(rawCounts);
  const totalOccurrences = labels.reduce((acc, n) => acc + preferredTypes.reduce((inner, type) => inner + (rawCounts[n][type] || 0), 0), 0);
  const gradients = {
    'Crime': (() => { const g = ctx.createLinearGradient(0, 0, canvas.width || 420, 0); g.addColorStop(0, '#F87171'); g.addColorStop(1, '#DC2626'); return g; })(),
    'Acidente/Urbano': (() => { const g = ctx.createLinearGradient(0, 0, canvas.width || 420, 0); g.addColorStop(0, '#FBBF24'); g.addColorStop(1, '#D97706'); return g; })(),
    'Suspeito': (() => { const g = ctx.createLinearGradient(0, 0, canvas.width || 420, 0); g.addColorStop(0, '#22D3EE'); g.addColorStop(1, '#0891B2'); return g; })(),
    'Resolvido': (() => { const g = ctx.createLinearGradient(0, 0, canvas.width || 420, 0); g.addColorStop(0, '#34D399'); g.addColorStop(1, '#059669'); return g; })()
  };

  if (alertsByTypeAndNeighborhoodChart) {
    try { alertsByTypeAndNeighborhoodChart.destroy(); } catch (error) {}
  }

  alertsByTypeAndNeighborhoodChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels,
      datasets: preferredTypes.map(type => ({
        label: type,
        data: labels.map(n => rawCounts[n][type] || 0),
        backgroundColor: gradients[type],
        hoverBackgroundColor: baseColors[type],
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label(context) {
              ...
              return `${context.dataset.label}: ${count} ocorrência${count === 1 ? '' : 's'}`;
            },
            footer(items) {
        ...
        title: {
          ...
          padding: { bottom: 14 },
          text: totalOccurrences === 0
            ? 'Sem ocorrências para os filtros atuais'
            : (periodDays === 365 ? 'Tipos de alerta por bairro no último ano' : `Tipos de alerta por bairro nos últimos ${periodDays} dias`)
        }
      }
    }
  });
...
function setupThemeToggle() {
  ...
  const applyTheme = (theme) => {
    ...
    if (metaTheme) {
      metaTheme.setAttribute('content', theme === 'dark' ? '#081120' : '#f8fbff');
    }
    ...
    const selectedRadio = document.querySelector(`input[name="themeSelection"][value="${theme}"]`);
    if (selectedRadio) selectedRadio.checked = true;
  };

  applyTheme(getPreferredTheme());

  if (themeToggle && !themeToggle.dataset.boundTheme) {
...
/* ================================================= */
/* HERO STARTUP UI - MICRO INTERAÇÕES                */
/* ================================================= */
document.addEventListener("DOMContentLoaded", function(){
  const hero = document.querySelector(".saas-command-hero");
  const stats = document.querySelectorAll(".saas-command-stat");
  const buttons = document.querySelectorAll(".saas-command-actions .saas-hero-btn");
...
/* ================================================= */
/* CAIXA DE ENTRADA - MICROINTERAÇÕES SAAS           */
/* ================================================= */
document.addEventListener("DOMContentLoaded", function(){
  const messageViewLink = document.querySelector('[data-view="messages"]');
  if(messageViewLink){
    messageViewLink.addEventListener("click", function(){
      setTimeout(() => {
        document.querySelectorAll('.inbox-message-item').forEach((item, index) => {
          setTimeout(() => {
            item.style.transform = 'translateY(-2px)';
            setTimeout(() => { item.style.transform = ''; }, 220);
          }, index * 35);
        });
      }, 180);
    });
...
// Persistência leve da busca global para melhorar a UX do dashboard
(function(){
  const STORAGE_KEY = 'ca_global_search';
  document.addEventListener('DOMContentLoaded', () => {
